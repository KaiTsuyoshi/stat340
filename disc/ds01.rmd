...
```
   ...
   b. Find the mean of this vector and subtract it from each number. 
...
```
   ...
   c. Using `>=`, compare this vector with 0 to show if each number is greater than or equal to 0. Use `sum()` on this resultant vector to count how many numbers satisfy this criterion (or alternatively, use `mean()` to get the proportion (think about why this works!)).
...
```{r}
dice = function(n = 2) {
   trial = sample(1:6, size = n, replace = TRUE)
   return(mean(trial))
}
```
...
```{r}
std_dev = function(vect) {
   mean = mean(vect)
   len = length(vect)
   
   s = (1/(len-1)) * sum((vect - mean)^2)
   sqrt(s)
}
