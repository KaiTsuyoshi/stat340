...
diffs %>% enframe(name=NULL,value="diff") %>%
  ggplot(aes(x=diff)) + geom_density(adjust=2) + geom_vline(xintercept=167)
pval = mean(diffs >= 167)
```
...
for (i in 1:10000) {
   kappa = summary(lm(mpg ~ sample(drat), data=mtcars.small))
   beta_1[i] = kappa$coefficients[2,1]
   se_1 = kappa$coefficients[2,2]
   p_values[i] <- 2*(1-pt((beta_1[i] - 0 )/se_1,length(mtcars$mpg) - 2))
}
beta_1 %>% enframe(name=NULL,value="beta_1") %>%
  ggplot(aes(x=beta_1)) + geom_density(adjust=2) + geom_vline(xintercept=167)
(pval = mean(beta_1 >= 5.096042))
# kappa = summary(lm(mpg ~ drat, data=mtcars.small))
# beta_1 = kappa$coefficients[2,1]
# se_1 = kappa$coefficients[2,2]
# p_values[i] <- 2*(1-pt((beta_1 - 0 )/se_1,length(mtcars$mpg) - 2))
```
