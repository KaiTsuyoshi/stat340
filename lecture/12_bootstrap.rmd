...
<!-- ```{r} -->
<!-- form = mpg ~ hp + wt + disp + cyl + qsec + I(hp^2) + gear +  -->
<!--   wt:cyl + wt:qsec + wt:gear + hp:qsec + qsec:gear -->

<!-- lm1 = lm(form,mtcars) -->
<!-- nterms = length(attr(terms(form),"term.labels"))+1 -->

<!-- par(mfrow=c(1,2)) -->
<!-- plot(lm1,which=1:2,ask=F) -->

<!-- se.lm = summary(lm1)$coefficients[,2] -->
<!-- se.lm -->

<!-- B = 10000 -->
<!-- coefs.boot = matrix(NA,nrow=B,ncol=nterms) -->

<!-- for (i in 1:B){ -->
<!--   rows = sample(nrow(mtcars),replace=T) -->
<!--   coefs.boot[i,] = coef(lm(form,mtcars[rows,])) -->
<!-- } -->

<!-- library(Rfast) -->

<!-- se.boot = sqrt(colVars(coefs.boot[complete.cases(coefs.boot),])) -->
<!-- se.boot -->

<!-- se.lm -->
<!-- se.boot -->

<!-- 100*((se.lm-se.boot)/se.boot) -->
<!-- ``` -->
