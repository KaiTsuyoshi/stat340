...
```{r}
# simulate poisson, make bootstrap to simulate, find m^2/k in each simulation, find point estimate and 90% 
replic = rep(NA, 200)
for(i in 1 : 200){
  sample = rpois(200, lambda = 250)
  replic[i] = (sample[i]^2)/14
}
pt_est = mean(replic)
pt_est
se = sd(replic)
CI = c(pt_est - 1.645* se, pt_est + 1.645 * se)
CI
