...
mc.derange = function(n,i=1000){
  counter = 0
  for(loop in 1:i){
    orig = sample(1:n, size = n, replace = FALSE)
    coff = sample(1:n, size = n, replace = FALSE)
    if (all(orig != coff)){
      counter = counter + 1
    }
...
ggplot(df.derange)+
  geom_point(aes(x = n, y = theory, color = "theory"))+
  geom_point(aes(x = n, y = mc, color = "mc"))+
  xlab("Number of Students") +
  ylab("Probability of No Repeats")+
  ggtitle("Theory vs. Monte Carlo Results of Handing out Tests")
```
...
```{r}
tibble(values = c(samp1,samp2), group = c(rep("samp1",100), rep("samp2", 100))) %>% 
  ggplot(aes(x = values, fill = group, color = group)) +
  geom_histogram() 
```
...
Our null hypothesis is that the two are of the same distribution, that mean of samp1 = mean of samp2, and our alternative that mean of samp1 is not the same as mean of samp2. Based on the t test, we see that the p-value is 0.3998 > 0.05, which means that we fail to reject the null that the means are different, and thus prove that the two are taken from the same data set. 
...
sd_smp = sd(samp1)
n = length(samp1)
error = qt((0.95 + 1)/2, df = n - 1) * sd_smp / sqrt(n)
result = c("lower" = mean_smp - error, "upper" = mean_smp + error)
...
```{r}
attitude
prcomp(attitude, scale = TRUE)
biplot(prcomp(attitude, scale = TRUE))
```

Only 0.175 of information is captured in the first 2 axes

We need to keep at least four axes to maintain 90% of the data set.
